...
# Function to create a new MSA - MMMMULTRA MINIMAL VERSION
function Create-MSA {
    Clear-Host
    ...
    try {
        ...
        switch ($modOption) {
            "1" {
                ...
                $computer = Read-Host "Enter the new computer name to assign to this MSA"
            ...
            "2" {
                ...
                if ($selectedMSA.objectClass -contains "msDS-GroupManagedServiceAccount") {
                    $adGroupName = Read-Host "Enter the AD group name to assign to this gMSA"
                    try {
                        Set-ADServiceAccount -Identity $selectedMSA.Name -PrincipalsAllowedToRetrieveManagedPassword $adGroupName
                        Write-Host "Assigned AD group '$adGroupName' to gMSA '$($selectedMSA.Name)'." -ForegroundColor Green
                    } catch {
